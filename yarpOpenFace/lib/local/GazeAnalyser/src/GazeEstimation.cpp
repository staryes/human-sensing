...
cv::Point3f RaySphereIntersect(cv::Point3f rayOrigin, cv::Point3f rayDir,
  ...
  if (b * b - 4 * a * c < 0) return cv::Point3f(0, 0, -1);
...
cv::Point3f GazeAnalysis::GetPupilPosition(cv::Mat_<float> eyeLdmks3d) {
  ...
  // std::cout << "pupil position: " << p.x << " " << p.y << " " << p.z << endl;
  // //test print
...
cv::Point3f GazeAnalysis::GetPupilPositionR1(cv::Mat_<float> eyeLdmks3d,
  ...
  // std::cout << "pupil position: " << mean(irisLdmks3d.col(0))[0] << " "
  //           << mean(irisLdmks3d.col(1))[0] << " " << mean(irisLdmks3d.col(2))[0]
  //           << endl;
  // std::cout << "nwpil position: " << p.x << " " << p.y << " " << p.z
  //           << endl;  // test print
...
void GazeAnalysis::EstimateGazeR1(const LandmarkDetector::CLNF& clnf_model,
  ...
  cv::Point3f pupil =
      GetPupilPositionR1(eyeLdmks3d, pupil_center, fx, fy, cx, cy);
  cv::Point3f rayDir = pupil / norm(pupil);
